...
export class CouriersService {
  ...
  async list(r: StoreRequester, q: { store_id?: string; is_active?: string; is_online?: string }) {
    const where: any = {};
  ...
  async cashBalance(courierId: number) {
    const [row]: any[] = await Courier.sequelize.query(
      `SELECT
         COALESCE((SELECT SUM(cash_collected) FROM deliveries WHERE courier_id = :id AND status = 'delivered'), 0)::bigint AS collected,
